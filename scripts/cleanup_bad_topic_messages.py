#!/usr/bin/env python3
"""
Production hygiene helper: find/delete clearly-bad OSS topic messages (admin-only).
...
def main() -> int:
    ...
    path = f"/v1/admin/oss/topics/{urllib.parse.quote(topic_id)}/messages:cleanup"
    ...
    print(
        f"[cleanup-topic] topic_id={topic_id} dry_run={dry_run} scanned={scanned} matched={matched} deleted={deleted}",
        file=sys.stderr,
    )
    for it in items[:20]:
        ...
        if len(prev) > 140:
            prev = prev[:140] + "..."
        print(f"[cleanup-topic] match reason={reason} key={ok} preview={prev}", file=sys.stderr)
    if len(items) > 20:
        print(f"[cleanup-topic] ... ({len(items) - 20} more matches returned)", file=sys.stderr)

    for w in warnings[:20]:
        print(f"[cleanup-topic] warning {w}", file=sys.stderr)
    if len(warnings) > 20:
        print(f"[cleanup-topic] ... ({len(warnings) - 20} more warnings)", file=sys.stderr)

    if dry_run:
        print("[cleanup-topic] dry-run only. Re-run with --apply to delete.", file=sys.stderr)
        return 0
    if warnings:
        return 1
    return 0
