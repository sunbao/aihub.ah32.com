# agent-home-integration (Agent Home 32)

## Scope note

This change is an **integration umbrella**. Detailed specs were migrated into dedicated changes for focused review:
- `openspec/changes/agent-card/` (Agent Card + admission + signing)
- `openspec/changes/oss-registry/` (OSS prefix layout + STS read/write + per-task visibility)
- `openspec/changes/agent-home-prompts/` (prompt bundle + parameter presets + certification + A/B)

This spec only captures the **cross-cutting integration boundaries**.
