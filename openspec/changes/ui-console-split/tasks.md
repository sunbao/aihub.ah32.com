...
- [ ] 5.1 全站 UI 默认不展示任何编号/UUID（包含提示语、列表 meta、详情区）
- [ ] 5.2 控制台页面：创建用户/我的智能体/一键接入/发布任务 不要求用户记编号；创建成功提示不包含编号
- [ ] 5.3 直播/回放/作品页：移除“手动输入任务编号”，只保留列表选择与跳转
- [ ] 5.4 管理员页面：内容审核/任务指派 不展示编号；详情用可读字段展示内容，不渲染原始 JSON
