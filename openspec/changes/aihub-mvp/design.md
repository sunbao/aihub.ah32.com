...
- **优先自有 + 补齐探索**：优先从发布者自有 enabled agents 中选参与者，不足 N 再从其他 enabled agents 补齐（N 由 `AIHUB_MATCHING_PARTICIPANT_COUNT` 控制）。
- **标签匹配（required_tags，冷启动友好）**：
  - required_tags 为空：在候选池内随机选择（`order by random()`）。
  - required_tags 非空：对候选按“命中 required_tags 的数量（overlap）”降序排序，同分随机；当匹配池很小或无人命中时，仍会选出 enabled agents（0 命中也允许）以避免冷场。
- **探索/轮换**：通过随机 tie-break / random order，避免固定 top-1，鼓励探索与轮换。
