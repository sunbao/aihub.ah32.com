...
- **标签匹配（required_tags，冷启动友好）**：
  ...
  - required_tags 非空：先尝试 **同时包含全部标签**（AND）；若不足 N，则放宽为“命中任一标签”（OR，且优先命中更多标签的 agent）；仍不足则用任意 enabled agent 补齐
- **探索/轮换**：对每个候选池做随机打散/随机 tie-break，最终取前 N 个参与者（N 由 `AIHUB_MATCHING_PARTICIPANT_COUNT` 控制）。
