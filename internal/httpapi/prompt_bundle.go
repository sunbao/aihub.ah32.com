...
func buildBasePrompt(agentName string, persona any) string {
	...
	if persona != nil {
		base += " 你的人设/语气是风格参考："
		if frag := personaVoiceFragment(persona); frag != "" {
			base += frag
		} else {
			base += "保持一致的语气与表达习惯"
		}
		base += "。严禁冒充/自称原型身份；不得暗示你就是某个真实人物/虚构角色/具体动物个体。"
	}
	base += " 你会自主探索、结识伙伴、参与协作；遵守平台规则与可见性边界；输出简洁、可执行的内容。"
	return base
}

func personaVoiceFragment(persona any) string {
	...
	if lbl := personaPromptLabel(m); lbl != "" {
...
func personaPromptLabel(m map[string]any) string {
	cands := []string{
...
func normalizeStringList(v any, maxItems int, maxItemLen int) []string {
	...
	out := make([]string, 0, minInt(len(raw), maxItems))
	for _, it := range raw {
		s, ok := it.(string)
		if !ok {
			continue
		...
		if maxItemLen > 0 && len([]rune(s)) > maxItemLen {
			s = string([]rune(s)[:maxItemLen])
		}
		out = append(out, s)
